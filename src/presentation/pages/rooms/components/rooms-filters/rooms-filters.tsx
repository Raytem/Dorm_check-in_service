...
const RoomsFilters: React.FC<RoomFiltersProps> = ({
	...
	const [debouncedRoomName] = useDebouncedValue(localRoomName, DEBOUNCE_DELAY);

	const [localFloor, setFloor] = useState(floor);
	const [debouncedFloor] = useDebouncedValue(localFloor, DEBOUNCE_DELAY);

	const [localDormitoryNumber, setDormitoryNumber] = useState(dormitoryNumber);
	const [debouncedDormitoryNumber] = useDebouncedValue(localDormitoryNumber, DEBOUNCE_DELAY);

	const [localBlockNumber, setBlockNumber] = useState(blockNumber);
	const [debouncedBlockNumber] = useDebouncedValue(localBlockNumber, DEBOUNCE_DELAY);

	const [localStudentGroup, setStudentGroup] = useState(studentGroup);
	const [debouncedStudentGroup] = useDebouncedValue(localStudentGroup, DEBOUNCE_DELAY);

	const [localOnlyAvailableRooms, setOnlyAvailableRooms] = useState(onlyAvailableRooms);
	const [localBlockType, setBlockType] = useState(blockType);

	useEffect(() => {
		setFilters({
			roomName: debouncedRoomName,
			floor: debouncedFloor,
			dormitoryNumber: debouncedDormitoryNumber,
			blockNumber: debouncedBlockNumber,
			studentGroup: debouncedStudentGroup,
		...
		debouncedRoomName,
		debouncedFloor,
		debouncedDormitoryNumber,
		debouncedBlockNumber,
		debouncedStudentGroup,
		localOnlyAvailableRooms,
		localBlockType,
		setFilters
	]);
	...
	const isFiltersEmpty = Object.values(filters)
