...
import { AppShell, Box, Title, TitleOrder, useMatches } from '@mantine/core';
...
import ProfileBlock from '@components/app-layout/profile-block';
...
const Header: React.FC<HeaderProps> = ({ burgerOpened, toggleBurger }) => {
  ...
  return (
    <AppShell.Header bg={'main.6'} c={'white'}>
      <Box className={classes['header__inner']!}>
        <Box className={classes['header__logo']!}>
          <Link to={AppRoutes.HOME}>
            <Title order={logoTitleOrder} lineClamp={2} textWrap={'wrap'}>
              {config.getConfig().app.title}
            </Title>
          </Link>
        </Box>

        <Box className={classes['header__toolbar']!}>
          ...
          {authenticatedUser && (
            <>
              ...
              <LogoutButton
                visibleFrom={'sm'}
                className={classes['header__toolbarLogoutBtn']!}
              />
