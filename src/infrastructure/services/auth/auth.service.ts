...
export class AuthService implements IAuthService {
  ...
  async login(dto: LoginDto): Promise<LoginResponse> {
    try {
      const response =
        await this.authApiHttpService.instance.post<AuthApi.LoginResponse>(
          '/auth/admin/login',
          dto,
        );
  ...
  async logout(): Promise<void> {
    try {
      await this.authApiHttpService.instance.post(
        '/auth/logout',
        {},
        {
  ...
  async refreshTokens(): Promise<TokensResponse> {
    ...
    try {
      ...
      const response = await axios.post<AuthApi.RefreshTokensResponse>(
